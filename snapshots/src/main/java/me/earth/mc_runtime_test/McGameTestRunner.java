...
public class McGameTestRunner {
    ...
    public static MultipleTestTracker runGameTests(UUID playerUUID, MinecraftServer server) throws ExecutionException, InterruptedException, TimeoutException {
        return server.submit(() -> {
            ...
            Collection<GameTestInfo> tests = GameTestRegistry.getAllTestFunctions()
                                                             .stream()
                                                             .filter(f -> !f.manualOnly())
                                                             .map(testFunction -> new GameTestInfo(testFunction, rotation, level, RetryOptions.noRetries()))
                                                             .toList();
